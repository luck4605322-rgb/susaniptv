...
LANGUAGES = {
    "简体中文": {
        "title": "IPTVNator 批量检测工具 v3.8 - 纯浏览器本地检测",
        "username": "用户名:",
        "password": "密码:",
        "servers": "服务器地址（一行一个）:",
        "example": "填入示例",
        ...
        "footer": "v3.8 纯浏览器本地检测 • 最后优化版",
        "warning": "请填写服务器列表、账号和密码！",
        "cors_warning": "⚠️ 注意：这是纯浏览器本地检测。绝大多数 IPTV 服务器会因 CORS 限制显示“连接失败”。这是浏览器安全机制，不是代码或服务器问题。",
        "running": "🚀 从您的浏览器本地网络开始检测 {0} 个服务器...",
        "detecting": "[{0}/{1}] 检测中 → {2}",
        "complete": "✅ 纯浏览器本地检测完成！共检测 {0} 个服务器。",
...
st.set_page_config(page_title="IPTVNator 纯浏览器检测 v3.8", layout="wide", page_icon="🚀")
...
if st.button(trans["start_btn"], type="primary", use_container_width=True):
    ...
    <head>
        ...
        <style>
            ...
            .panel {{ max-width:1150px; margin:auto; background:white; border-radius:16px; box-shadow:0 10px 50px rgba(0,0,0,0.2); padding:35px; }}
            .progress-container {{ height:42px; background:#e9ecef; border-radius:12px; overflow:hidden; margin:30px 0; }}
            .progress-bar {{ height:100%; background:linear-gradient(90deg, #28a745, #20c997); width:0%; transition:width 0.8s ease; }}
            .log {{ background:#f8f9fa; border:2px solid #ced4da; padding:20px; height:550px; overflow-y:auto; font-family:Consolas,monospace; white-space:pre-wrap; font-size:14.5px; line-height:1.7; border-radius:10px; }}
            .title {{ color:#28a745; margin-top:0; }}
        </style>
    </head>
    <body>
    <div class="panel">
        <h2 class="title">🚀 {running_text}</h2>
